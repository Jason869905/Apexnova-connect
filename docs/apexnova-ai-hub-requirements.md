...
Connect 当时的处置（不等 Hub）：`recommend` 把「`input` 与 `output` 都为零」判定为**没有价格**而不是零价；当本轮所有候选都没有可用价格时，整个 `cost` 分项被丢弃——按 [ADR 0007](decisions/0007-m4-scope-and-recommendation-path.md) 的规则，没有度量来源的分项不参与排序，也不给默认分。

修好之后 Connect 无需改代码：非零价格会自动重新参与排序。另一条备选路径是逐 Deployment 调估价接口取价，但那是 46 次请求换一次排序，只有在目录长期不提供价格时才值得做。

**2026-09-11：本条已交付，并已消费。** Hub 按上面两条路都走了——现网 46 个 Deployment 中 34 个给出真实价格、5 个零价、7 个 `input`/`output` 为 `null`。两个后果：

- `cost` **第一次真正参与排序**（此前每轮都被整项丢弃），权重随之重新分配，OpenCode 的第一名由 GLM-5.1 变为 DeepSeek-V4-Pro-0813；`--max-price` 也第一次真的在筛（`1.0` 留 2 个、`0.5` 留 1 个）；
- **零价的语义随之恢复**：`null` 现在承担「本投影不提供价格」，因此一个发布出来的 `0` 意味着免费，`recommend` 按免费打分（`cost` 满分）。现网 5 个零价 Deployment 中有一个就叫 `Cohere: North Mini Code (free)`。

同时暴露了 Connect 的一处缺陷，已修：我们在本需求里给了 Hub 两个可接受形状，却只实现了第一个——**第一个 `null` 让整份目录解析失败**，`models`、`recommend`、`connect`、`run` 全线不可用。给对方留选择，意味着必须实现对方可能选的每一个。
