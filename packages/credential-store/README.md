...
macOS Keychain 后端尚未实现。`createDefaultCredentialStore()` 在不支持的平台会明确抛出 `UNSUPPORTED_PLATFORM`，不会把秘密写入明文文件。Windows 需要交互式用户登录会话关联的 credential set；Linux 需要系统已安装 `secret-tool` 且存在可用的 Secret Service 会话。缺少这些条件时返回 `BACKEND_UNAVAILABLE`。
