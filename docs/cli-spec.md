...
目标文件在写入后被改动（Agent 规范化自己的配置是常态）或被删除时，`restore` 以 `CONFLICT` 拒绝——这是对的，但必须有出路：`--discard-local-changes` 先把**当前文件原样复制**到 `<state>/backups/discarded/`，再执行恢复，并在输出里给出副本位置。**「丢弃」不销毁任何东西**；会销毁的出路只是换了个客气名字的陷阱。目标已被删除时没有可保留的内容，直接恢复。
