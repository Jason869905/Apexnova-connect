...
| macOS | Keychain（`security`） | 已实现，**从未在真实 macOS 上验收**，只有 mock command runner 测试。**macOS 已按 [ADR 0024](decisions/0024-narrow-platform-claims.md) 移出支持范围**：这段实现保留，但不作为受支持路径 |
