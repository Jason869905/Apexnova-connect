...
- `restore` 的顺序判定以绑定为准而非以备份元数据为准。绑定与备份不一致时（旧版本会产生），较新的事务被顺序检查拒绝、较旧的被内容哈希检查拒绝，CLI 完全没有出路。已修复（2026-09-08，顺序改由备份元数据推导，绑定只留给凭据链；见 [路线图](../roadmap.md)）。
- 被拒绝的回滚会把事务标记成 `rolling-back`，而实际上一个字节都没动——恢复扫描与 `restore --list` 都会照着这个错误状态理解。已修复（校验通过后才移动状态标记）。
